...
func PayloadUnaryServerInterceptor(entry *logrus.Entry, decider grpc_logging.ServerPayloadLoggingDecider) grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
		...
		labelsEntry := entry.WithFields(labels)
		// Use the provided logrus.Entry for logging but use the fields from context.
		logEntry := labelsEntry.WithFields(ctxlogrus.Extract(ctx).Data)
...
func PayloadStreamServerInterceptor(entry *logrus.Entry, decider grpc_logging.ServerPayloadLoggingDecider) grpc.StreamServerInterceptor {
	return func(srv interface{}, stream grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) error {
		...
		labels := logrus.Fields{}
		tx := apm.TransactionFromContext(stream.Context())
		if tx != nil {
			traceContext := tx.TraceContext()
			labels["trace.id"] = traceContext.Trace.String()
			labels["transaction.id"] = traceContext.Span.String()
			if span := apm.SpanFromContext(stream.Context()); span != nil {
				labels["span.id"] = span.TraceContext().Span.String()
			}
		}
		...
		labelsEntry := entry.WithFields(labels)
		// Use the provided logrus.Entry for logging but use the fields from context.
		logEntry := labelsEntry.WithFields(ctxlogrus.Extract(stream.Context()).Data)
...
func PayloadStreamClientInterceptor(entry *logrus.Entry, decider grpc_logging.ClientPayloadLoggingDecider) grpc.StreamClientInterceptor {
	return func(ctx context.Context, desc *grpc.StreamDesc, cc *grpc.ClientConn, method string, streamer grpc.Streamer, opts ...grpc.CallOption) (grpc.ClientStream, error) {
		...
		logEntry := labelsEntry.WithFields(newClientLoggerFields(ctx, method))
